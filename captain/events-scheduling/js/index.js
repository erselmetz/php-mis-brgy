/**
 * Events & Scheduling — Captain View Only
 * NO Add / NO Edit / View Only
 */
...
$(function () {
        ...
        filtered.forEach(ev => {
            ...
            $list.append(`
                <div class="ev-card ${priCls}" data-id="${ev.id}">
                    ...
                    <!-- Captain view: No action buttons -->
                </div>
            `);
        });
    }
    ...
    /* Sidebar search + priority filter */
    let searchTimer;
        ...
        for (let d = 1; d <= daysInMon; d++) {
            ...
            html += `
                <div class="cal-day ${isToday ? 'today' : ''}" data-date="${dateStr}">
                    ...
                    ${dayEvs.length > 5 ? `<div style="font-size:8px;color:var(--ink-faint);">+${dayEvs.length-5}</div>` : ''}
                </div>`;
        }
    ...
    /* Captain cannot add event by clicking calendar */
    $('#calGrid').on('click', '.cal-day:not(.other-month)', function () {
        showAlert('View Only', 'Only the secretary can create or edit events.', 'warning');
    });
    ...
    function initDataTable() {
        evTable = $('#eventsTable').DataTable({
            ajax: {
                ...
                dataSrc: json => (json && json.status === 'ok') ? json.data : []
            },
            dom: 'tip',
            order: [[2, 'asc']],
            pageLength: 50,
            ...
            columns: [
                ...
                { data:'event_date', render: d => `<span style="font-family:var(--f-mono);">${fmtDate(d)}</span>` },
                { data:'event_time', render: d => d ? fmtTime(d) : '—' },
                { data:'location', render:d => d ? esc(d) : '—' },
                { data:'priority', render:d => `<span class="pri-badge ${PRI_CONF[d]?.cls}">${PRI_CONF[d]?.label}</span>` },
                { data:'status', render:d => `<span class="ev-status ${STA_CONF[d]?.cls}">${STA_CONF[d]?.label}</span>` },
    ...
    function loadHistory(search) {
        ...
        $.getJSON('actions/fetch_event_history.php', { search: search || '', limit: 80 }, function (res) {
            ...
            if (!res || res.status !== 'ok' || !res.data || !res.data.length) {
                $list.html('<div style="padding:20px;text-align:center;color:var(--ink-faint);font-style:italic;">No history found.</div>');
                return;
            }
            res.data.forEach(ev => {
                ...
                $list.append(`
                    <div class="hist-card">
                        ...
                        <div class="hist-body">
                            ...
                            ${ev.description ? `<div class="hist-card-meta">${esc(ev.description.substring(0,120))}${ev.description.length>120?'…':''}</div>` : ''}
                        </div>
                        <div style="font-size:10px;color:var(--ink-faint);text-align:right;">
    ...
    $('#btnPrintEvents').on('click', function () {
        ...
        w.document.write(`
            <html><head><title>Events — ${monthLabel}</title>
            <style>
                body { font-family:Arial; padding:20px; font-size:12px; }
                table { width:100%; border-collapse:collapse; }
                th { background:#2d5a27; color:white; padding:5px; }
                td { padding:5px; border-bottom:1px solid #ddd; }
            </style>
            </head><body>
            <h2 style="text-align:center;">Barangay Bombongan</h2>
            <h3 style="text-align:center;">Events & Scheduling — ${monthLabel}</h3>
            <br>
            <table>
                <thead>
                    <tr><th>#</th><th>Code</th><th>Title</th><th>Date</th><th>Time</th><th>Location</th><th>Priority</th></tr>
                </thead>
                <tbody>${rows || '<tr><td colspan="7">No events</td></tr>'}</tbody>
            </table>
    ...
    /* ═══════════════════════════════════════
       BOOT
    ═══════════════════════════════════════ */
    $('#btnNewEvent').hide(); // hide add button fully for captain
    loadEvents();
