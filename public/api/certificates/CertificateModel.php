...
class CertificateModel extends BaseModel {
    ...
    public function getWithResidentInfo($conditions = [], $params = [], $orderBy = null, $limit = null, $offset = null) {
        try {
            ...
            // error_log("CertificateModel SQL: " . $sql);
            // error_log("CertificateModel Params: " . json_encode($params));
            
            $stmt = $this->conn->prepare($sql);
            if (!$stmt) {
                error_log("CertificateModel getWithResidentInfo prepare error: " . $this->conn->error);
                return [];
            }
            
            if (!empty($params)) {
                // Determine types based on parameter values
                $types = '';
            ...
            if (!$stmt->execute()) {
                error_log("CertificateModel getWithResidentInfo execute error: " . $stmt->error);
                $stmt->close();
                return [];
            }
            ...
            // Debug logging (uncomment to debug)
            // error_log("CertificateModel Results: " . count($data) . " certificates found");
            
            return $data;
        } catch (Exception $e) {
            error_log("CertificateModel getWithResidentInfo exception: " . $e->getMessage());
            return [];
        }
    }
    ...
    public function count($conditions = [], $params = []) {
        try {
            ...
            $stmt = $this->conn->prepare($sql);
            if (!$stmt) {
                error_log("CertificateModel count prepare error: " . $this->conn->error);
                return 0;
            }
            ...
            if (!$stmt->execute()) {
                error_log("CertificateModel count execute error: " . $stmt->error);
                $stmt->close();
                return 0;
            }
            ...
            error_log("CertificateModel count exception: " . $e->getMessage());
            return 0;
        }
    }
