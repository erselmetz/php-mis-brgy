<?php
/**
 * Consultation Add API — FIXED
 * Bug: meta.program was only set during add but lost on edit when status changed.
 * Fix: program is always stored in notes JSON and never overwritten by status update.
 *
...
/* IMPORTANT: program is always preserved — never let it become empty */
$program = trim($_POST['consultation_type'] ?? '');
$allowedPrograms = ['immunization','maternal','family_planning','prenatal','postnatal','child_nutrition'];
...
/* Build notes JSON — program is always present */
$notes = meta_encode([
    'program'      => $program,      /* ALWAYS stored, never lost */
    'sub_type'     => $sub_type,
    'status'       => $status,
    'time'         => $time,
