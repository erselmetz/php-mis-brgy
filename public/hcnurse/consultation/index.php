...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if ($action === 'add_consultation') include_once __DIR__ . '/add.php';
}
...
function unpackNotes(string $raw): array {
...
<head>
    ...
    *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
    ...
    .con-table-wrap .dataTables_wrapper { padding:0; font-family:var(--f-sans); }
    .con-table-wrap .dataTables_filter,
    .con-table-wrap .dataTables_length  { display:none; }
    .con-table-wrap .dataTables_info {
    ...
    #consultTable tbody tr:last-child { border-bottom:none; }
    ...
    .view-header-name {
        font-family:var(--f-serif); font-size:17px; font-weight:600; color:var(--ink);
    }
    ...
    .view-val { font-size:13px; font-weight:500; color:var(--ink); line-height:1.6; }
    ...
    .ui-autocomplete {
        border:1.5px solid var(--rule-dk) !important; border-radius:2px !important;
        box-shadow:0 4px 20px rgba(0,0,0,.12) !important;
        font-family:var(--f-sans) !important; font-size:13px !important;
        max-height:240px; overflow-y:auto !important;
        background:var(--paper) !important;
    }
    .ui-menu-item-wrapper {
        padding:9px 13px !important; border-bottom:1px solid #f0ede8 !important;
    ...
    /* Generate dialog */
    .gen-form { padding:18px 20px; display:flex; flex-direction:column; gap:14px; }
    </style>
</head>
<body class="bg-gray-100 h-screen overflow-hidden" style="display:none;">
            ...
            <div class="doc-header">
                <div class="doc-header-inner">
                    <div>
                        ...
                        <div class="doc-sub">Patient consultations and health visit logs</div>
                    </div>
                ...
                <table id="consultTable" class="display" style="width:100%;">
                    ...
                    <tbody>
                        <?php if ($result): while ($row = $result->fetch_assoc()):
                            ...
                            $typeCls = 'tb-' . ($prog ?: 'default');
                            if (!in_array($prog, ['immunization','maternal','family_planning','prenatal','postnatal','child_nutrition']))
                                $typeCls = 'tb-default';
                        ?>
                        <tr>
                            <td>
                            ...
                            <td>
                                <div class="td-actions">
                                    ...
                                    <button class="act-btn act-gen genConsultBtn"   data-resident-id="<?= (int)$row['resident_id'] ?>">Generate</button>
                                </div>
                            </td>
                        </tr>
                        <?php endwhile; else: ?>
                    ...
                    <div class="form-grid-2">
                        ...
                        <div class="fg">
                            ...
                            <input type="text" name="sub_type" class="fg-input"
                                   placeholder="e.g. BCG, Pills, Mother Only…" autocomplete="off">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-section-body">
                    <div class="fg">
                        ...
                        <input type="text" id="edit_resident_name" class="fg-input"
                               disabled style="background:var(--paper-lt);color:var(--ink-muted);">
                    </div>
                </div>
            </div>
    ...
    $(function () {
        ...
        const table = $('#consultTable').DataTable({
            ...
            dom: 'tip',
            language: {
                info: 'Showing _START_–_END_ of _TOTAL_ consultations',
        ...
        $('#addConsultForm').on('submit', function(e){
            ...
            if(!$('#add_resident_id').val()){
                showAlert('Validation', 'Please select a resident from the dropdown.', 'danger'); return;
            }
            $.ajax({
        ...
        $(document).on('click','.viewConsultBtn', function(){
            ...
            $.getJSON('api/view.php',{ id }, function(res){
                ...
                $('#vc-time').text(d.time || '—');
                $('#vc-type').text(d.notes ? (JSON.parse(d.notes||'{}').program||'—').replace(/_/g,' ') : '—');
        ...
        $(document).on('click','.editConsultBtn', function(){
            ...
            $.getJSON('api/view.php',{ id }, function(res){
                ...
                /* date: yyyy-mm-dd → mm/dd/yyyy for datepicker */
        ...
        /* ════════════════════════
           GENERATE (per-row)
        ════════════════════════ */
        function openGenerateDialog(residentId, reportMode){
            $('#gen_resident_id').val(residentId || '');
            /* if report mode — hide resident-specific options */
            if(reportMode){
                $('#gen_doc').val('report');
                $('option[value="summary"], option[value="certificate"]').hide();
            } else {
                $('option[value="summary"], option[value="certificate"]').show();
                $('#gen_doc').val('summary');
            }
            $('#gen_doc').trigger('change');
        ...
        $('#gen_period').on('change', function(){
            $('#gen_month_wrap').toggle($(this).val() === 'monthly');
        });
        $('#gen_doc').on('change', function(){
            $('#gen_purpose_wrap').toggle($(this).val() === 'certificate');
        });

        /* Per-row generate button */
        $(document).on('click','.genConsultBtn', function(){
            openGenerateDialog($(this).data('resident-id'), false);
        });

        /* Top-level report button */
