$(function(){
    ...
    $('#cf_res_name').on('input',function(){
        if(!$(this).val()) { $('#cf_res_id').val(''); $('#patientCard').hide(); }
    });
    ...
    /* ── View modal ── */
    $('#viewModal').dialog({ autoOpen:false, modal:true, width:900, resizable:true,
    ...
    window.editConsult = function(id){
        $.getJSON('api/view.php',{id},function(res){
            ...
            $('[name="consult_type"]').val(d.consult_type||'general');
