...
/**
 * HC Nurse Sidebar — v3
 * - Appointments link added below Consultation
 * - Care Records route → /hcnurse/care-visits/?type=X
...
?>
<style>
.sb-root{width:232px;min-width:232px;background:#fdfcf9;border-right:1px solid #d8d4cc;display:flex;flex-direction:column;height:100%;overflow-y:auto;overflow-x:hidden;font-family:'Source Sans 3','Segoe UI',sans-serif;position:relative;flex-shrink:0;scrollbar-width:thin;scrollbar-color:#e0dcd6 transparent;}
.sb-root::after{content:'';position:absolute;top:0;right:0;bottom:0;width:1px;background:linear-gradient(to bottom,#c8c4bc,#e8e4de 40%,#c8c4bc);pointer-events:none;}
...
.sb-rule{flex:1;height:1px;background:#e0dcd6;}
.sb-item{display:flex;align-items:center;gap:11px;padding:9px 16px 9px 14px;font-size:13px;font-weight:500;color:#3a3a3a;text-decoration:none;border-left:2px solid transparent;position:relative;transition:background .12s,border-color .12s,color .12s;line-height:1.2;}
...
<aside class="sb-root" role="navigation" aria-label="HC Nurse navigation">
    ...
    <a href="/hcnurse/appointments/"
       class="sb-item <?= hcActive('/hcnurse/appointments/') ? 'is-active' : '' ?>">
    ...
    <?php
    /* BUG FIX: $careType not $type — prevents overwriting parent page's $type variable */
