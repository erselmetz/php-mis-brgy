...
/* ── Patients: age groups computed from actual birthdates ── */
$patients = fetchRow($conn,
    "SELECT
        COUNT(*) total,
        SUM(TIMESTAMPDIFF(YEAR, birthdate, CURDATE()) BETWEEN 0  AND 0)  infant,
        SUM(TIMESTAMPDIFF(YEAR, birthdate, CURDATE()) BETWEEN 1  AND 12) child,
        SUM(TIMESTAMPDIFF(YEAR, birthdate, CURDATE()) BETWEEN 13 AND 17) teen,
        SUM(TIMESTAMPDIFF(YEAR, birthdate, CURDATE()) BETWEEN 18 AND 59) adult,
...
/* ── Medicine inventory (static, not period-filtered) ── */
...
$medicineRows = [];
if (tableExists($conn,'medicines')) {
    $mr = $conn->query("SELECT name, stock_qty, reorder_level FROM medicines ORDER BY (stock_qty <= reorder_level) DESC, name ASC LIMIT 10");
...
<head>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    ...
    .doc-header-inner{padding:20px 28px 0;display:flex;align-items:flex-end;justify-content:space-between;gap:20px;flex-wrap:wrap;}
    .doc-eyebrow{font-size:8.5px;font-weight:700;letter-spacing:1.8px;text-transform:uppercase;color:var(--ink-faint);display:flex;align-items:center;gap:8px;margin-bottom:5px;}
    .doc-eyebrow::before{content:'';width:18px;height:2px;background:var(--accent);display:inline-block;}
    .doc-title{font-family:var(--f-serif);font-size:22px;font-weight:700;color:var(--ink);letter-spacing:-.3px;margin-bottom:3px;display:flex;align-items:baseline;gap:0;flex-wrap:wrap;}
    .doc-sub{font-size:12px;color:var(--ink-faint);font-style:italic;}
    #periodPill{display:inline-flex;align-items:center;padding:3px 10px;border-radius:2px;background:var(--accent-lt);border:1px solid color-mix(in srgb,var(--accent) 20%,transparent);font-family:var(--f-mono);font-size:9px;color:var(--accent);letter-spacing:.4px;margin-left:12px;vertical-align:middle;}

    /* period bar */
    .period-bar{display:flex;align-items:center;gap:6px;padding:12px 0 0;flex-wrap:wrap;}
    .period-lbl{font-size:8.5px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color:var(--ink-faint);margin-right:4px;}
    .pb-btn{padding:5px 13px;border-radius:2px;border:1.5px solid var(--rule-dk);background:#fff;font-size:10.5px;font-weight:700;letter-spacing:.3px;text-transform:uppercase;color:var(--ink-muted);cursor:pointer;transition:all .12s;font-family:var(--f-sans);}
    ...
    .pb-date{padding:5px 10px;border:1.5px solid var(--rule-dk);border-radius:2px;font-size:12px;font-family:var(--f-sans);color:var(--ink);background:#fff;outline:none;transition:border-color .14s;}
    .pb-date:focus{border-color:var(--accent);}
    .pb-apply{padding:5px 14px;border-radius:2px;background:var(--accent);border:1.5px solid var(--accent);color:#fff;font-size:10.5px;font-weight:700;letter-spacing:.4px;text-transform:uppercase;cursor:pointer;font-family:var(--f-sans);transition:filter .12s;}
    ...
    /* stat ledger */
    .stat-ledger{display:grid;grid-template-columns:repeat(5,1fr);margin:22px 28px 0;background:var(--paper);border:1px solid var(--rule);border-radius:2px;box-shadow:var(--shadow);overflow:hidden;}
    .sl-cell{padding:16px 18px;border-right:1px solid var(--rule);position:relative;transition:background .12s;}
    .sl-cell:last-child{border-right:none;}
    .sl-cell::after{content:'';position:absolute;top:0;left:0;right:0;height:3px;}
    .sl-0::after{background:var(--accent);} .sl-1::after{background:var(--blue);} .sl-2::after{background:var(--purple);} .sl-3::after{background:var(--teal);} .sl-4::after{background:var(--rose);}
    ...
    .sl-skel{height:30px;width:70px;border-radius:2px;background:linear-gradient(90deg,#f0ede8 25%,#e8e5e0 50%,#f0ede8 75%);background-size:300px 100%;animation:shimmer 1.2s infinite;}
    ...
    .hc-card-head{padding:10px 18px;border-bottom:1px solid var(--rule);background:var(--paper-lt);display:flex;align-items:center;justify-content:space-between;}
    .hc-card-title{font-size:8.5px;font-weight:700;letter-spacing:1.4px;text-transform:uppercase;color:var(--ink-muted);display:flex;align-items:center;gap:8px;}
    ...
    /* care grid - clickable cells */
    .care-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px;}
    .care-cell{
        padding:12px 14px;border-radius:2px;border:1px solid var(--rule);
        background:var(--paper-lt);position:relative;overflow:hidden;
        cursor:pointer; text-decoration:none; display:block;
        transition:transform .1s, box-shadow .1s;
    }
    .care-cell:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(0,0,0,.1);}
    ...
    .care-hint{font-size:9px;color:var(--ink-faint);margin-top:3px;font-style:italic;}
    ...
    .mf-good{background:var(--ok-fg);} .mf-avg{background:var(--amber);} .mf-low{background:var(--rose);} .mf-oos{background:var(--ink-faint);}
...
<body class="bg-gray-100 h-screen overflow-hidden" style="display:none;">
        ...
        <main class="flex-1 h-screen overflow-y-auto hcd-page">
            ...
            <div class="stat-ledger">
                ...
                <div class="sl-cell sl-4">
                    ...
                    <div class="sl-val" id="stLowStock" style="color:var(--rose);"><?= $inventory['low_stock'] ?></div>
                    <div class="sl-sub">of <strong><?= $inventory['items'] ?></strong> medicines</div>
                </div>
            </div>
            ...
            <div class="hcd-grid">
                <div class="hcd-left">
                    ...
                    <!-- Care Programs — each cell links to its health-records page -->
                    <div class="hc-card">
                        <div class="hc-card-head">
                            <span class="hc-card-title">Care Programs</span>
                            <span class="hc-card-meta" id="careCardMeta">Click a program to view records</span>
                        </div>
                        <div class="hc-card-body">
                            <div class="care-grid">
                                <a href="/hcnurse/health-records/?type=maternal" class="care-cell cc-maternal">
                                    <div class="care-val" id="cvMaternal">—</div>
                                    <div class="care-lbl">Maternal</div>
                                    <div class="care-hint">View records →</div>
                                </a>
                                <a href="/hcnurse/health-records/?type=family_planning" class="care-cell cc-fp">
                                    <div class="care-val" id="cvFP">—</div>
                                    <div class="care-lbl">Family Planning</div>
                                    <div class="care-hint">View records →</div>
                                </a>
                                <a href="/hcnurse/health-records/?type=prenatal" class="care-cell cc-prenatal">
                                    <div class="care-val" id="cvPrenatal">—</div>
                                    <div class="care-lbl">Prenatal</div>
                                    <div class="care-hint">View records →</div>
                                </a>
                                <a href="/hcnurse/health-records/?type=postnatal" class="care-cell cc-postnatal">
                                    <div class="care-val" id="cvPostnatal">—</div>
                                    <div class="care-lbl">Postnatal</div>
                                    <div class="care-hint">View records →</div>
                                </a>
                                <a href="/hcnurse/health-records/?type=child_nutrition" class="care-cell cc-nutrition">
                                    <div class="care-val" id="cvNutrition">—</div>
                                    <div class="care-lbl">Child Nutrition</div>
                                    <div class="care-hint">View records →</div>
                                </a>
                                <a href="/hcnurse/health-records/?type=immunization" class="care-cell cc-immune">
                                    <div class="care-val" id="cvImmune">—</div>
                                    <div class="care-lbl">Immunization</div>
                                    <div class="care-hint">View records →</div>
                                </a>
                            </div>
                            <div class="chart-h180"><canvas id="careChart"></canvas></div>
                        </div>
                    </div>
                    ...
                    <!-- Donuts — age groups with correct labels -->
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:18px;">
                        <div class="hc-card">
                            <div class="hc-card-head"><span class="hc-card-title">Residents by Age Group</span></div>
                ...
                <div class="hc-card" style="position:sticky;top:0;">
                    <div class="hc-card-head"><span class="hc-card-title">Medicine Inventory</span></div>
                    ...
                    <div class="med-row">
                        <div class="med-info">
                            ...
                            <div class="med-track"><div class="med-fill <?= $fillCls ?>" style="width:<?= $m['pct'] ?>%;"></div></div>
                        </div>
                    ...
                    <?php endif; ?>
                    <div class="inv-strip">
                        <div class="inv-strip-cell"><div class="inv-strip-val"><?= $inventory['items'] ?></div><div class="inv-strip-lbl">Items</div></div>
                        <div class="inv-strip-cell"><div class="inv-strip-val" style="color:var(--ok-fg);"><?= $inventory['ok_stock'] ?></div><div class="inv-strip-lbl">OK Stock</div></div>
                        <div class="inv-strip-cell">
                            <div class="inv-strip-val" style="color:<?= $inventory['low_stock'] > 0 ? 'var(--rose)' : 'var(--ink-faint)' ?>;"><?= $inventory['low_stock'] ?></div>
    ...
    $(function () {
        ...
        const PRESET_LABELS = { today:'TODAY', this_month:'THIS MONTH', last_30:'LAST 30 DAYS', this_year:'THIS YEAR', all_time:'ALL TIME' };

        let careInst=null, trendInst=null, patientInst=null, stockInst=null;
        function mkChart(inst,id,cfg){ if(inst)inst.destroy(); const el=document.getElementById(id); if(!el)return null; return new Chart(el,cfg); }
        ...
        function renderAll(d) {
            ...
            careInst = mkChart(careInst,'careChart',{
                ...
                data:{
                    ...
                    datasets:[{ data:[cv.maternal??0,cv.family_planning??0,cv.prenatal??0,cv.postnatal??0,cv.child_nutrition??0,d.immunizations??0], backgroundColor:['#fda4af','#93c5fd','#fcd34d','#5eead4',ACCENT2+'bb','#c4b5fd'], borderRadius:3 }]
                },
            ...
            trendInst = mkChart(trendInst,'trendChart',{
                type:'line',
                data:{ labels:days.length?days:['No data'], datasets:[{ label:'Consultations', data:counts.length?counts:[0], borderColor:ACCENT, backgroundColor:ACCENT+'22', fill:true, tension:.4, pointRadius:3, pointBackgroundColor:ACCENT, borderWidth:2 }] },
        ...
        function renderStaticCharts() {
            /* Real age groups from DB — Infant / Child / Teen / Adult / Senior */
            const p   = <?= json_encode($patients) ?>;
            const inv = <?= json_encode($inventory) ?>;
            patientInst = mkChart(patientInst,'patientChart',{
                type:'doughnut',
                data:{
                    labels:['Adult (18–59)','Senior (60+)','Teen (13–17)','Child (1–12)','Infant (<1)'],
                    datasets:[{ data:[p.adult,p.senior,p.teen,p.child,p.infant], backgroundColor:[ACCENT,'#f59e0b','#3b82f6','#a78bfa','#fda4af'], borderWidth:1.5, borderColor:'#fff' }]
                },
        ...
        $('.pb-btn').on('click',function(){
            const key=$(this).data('preset'), [f,t]=presets[key]();
            $('.pb-btn').removeClass('active'); $(this).addClass('active');
            $('#pbFrom').val(f); $('#pbTo').val(t);
            load(f,t,key);
        });
        $('#pbApply').on('click',function(){
            const f=$('#pbFrom').val(),t=$('#pbTo').val(); if(!f||!t)return;
            $('.pb-btn').removeClass('active'); load(f,t,null);
        });
