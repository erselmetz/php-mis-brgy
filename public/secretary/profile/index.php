    ...
    if (!isset($_POST['csrf_token']) || !validateCSRFToken($_POST['csrf_token'])) {
        ...
        if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
            ...
            $validation = validateUploadedFile(
                ...
                ['image/jpeg','image/png','image/gif'], 2097152, ['jpg','jpeg','png','gif']
            );
            if (!$validation['valid']) {
...
if (!empty($user['profile_picture'])) {
    ...
    if (file_exists($picPath)) $picSrc = '/uploads/profiles/' . htmlspecialchars($user['profile_picture']);
}
...
<head>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .doc-header-inner { padding: 20px 28px 20px; }
    .doc-eyebrow {
    ...
    .set-card + .set-card { margin-top: 22px; }
    .card-head {
        padding: 14px 20px;
    ...
    .card-title::before {
        content: ''; display: inline-block; width: 3px; height: 14px; background: var(--accent); border-radius: 1px;
    }
    ...
    /* Avatar */
    .avatar-zone { display: flex; flex-direction: column; align-items: center; padding: 24px 20px 20px; border-bottom: 1px solid var(--rule); }
    .avatar-ring {
    ...
    .avatar-ring:hover { box-shadow: 0 0 0 6px var(--accent-lt); }
    .avatar-img {
    ...
    .avatar-role {
        ...
        text-transform: uppercase; color: var(--ink-faint);
        background: var(--neu-bg); padding: 3px 10px; border-radius: 2px;
        border: 1px solid var(--rule);
    }
    .avatar-meta {
        margin-top: 10px; font-family: var(--f-mono); font-size: 10px;
        color: var(--ink-faint); letter-spacing: .3px; text-align: center;
    }
    ...
    .fg-file-btn {
        ...
        background: var(--paper-lt); text-align: center;
        font-size: 11.5px; color: var(--ink-muted); cursor: pointer;
        transition: border-color .15s, color .15s;
    }
    .fg-file-btn:hover { border-color: var(--accent); color: var(--accent); }
    .fg-file-name { font-size: 10.5px; color: var(--ok-fg); margin-top: 5px; font-family: var(--f-mono); }
    ...
    .rep-table tbody tr { border-bottom: 1px solid #f0ede8; transition: background .1s; }
    .rep-table tbody tr:last-child { border-bottom: none; }
    .rep-table tbody tr:hover { background: var(--paper-lt); }
    .rep-table td { padding: 11px 14px; font-size: 12.5px; color: var(--ink); vertical-align: middle; }
    .rep-label { font-weight: 600; }
    .rep-count { font-family: var(--f-mono); font-size: 14px; font-weight: 600; color: var(--ink); }
    .rep-time  { font-size: 11px; color: var(--ink-faint); font-style: italic; }

    /* Backup archive visual */
    .backup-panel { display: grid; grid-template-columns: 1fr 200px; gap: 20px; align-items: start; }
    .backup-icon-block {
        display: flex; flex-direction: column; align-items: center;
        padding: 20px; border: 1.5px dashed var(--rule-dk); border-radius: 2px;
        background: var(--paper-lt); gap: 14px;
    }
    .backup-icon {
    ...
    .bk-table tbody tr { border-bottom: 1px solid #f0ede8; transition: background .1s; }
    .bk-table tbody tr:hover { background: var(--paper-lt); }
    .bk-table td { padding: 10px 14px; vertical-align: middle; }
    .bk-date { font-family: var(--f-mono); font-size: 11.5px; color: var(--accent); font-weight: 600; }
    .bk-time { font-family: var(--f-mono); font-size: 10.5px; color: var(--ink-faint); margin-top: 1px; }
    .bk-size { font-family: var(--f-mono); font-size: 11.5px; color: var(--ink-muted); }
    .bk-by   { font-size: 12px; color: var(--ink-muted); }
    .bk-desc { font-size: 11px; color: var(--ink-faint); font-style: italic; }
...
<body class="bg-gray-100 h-screen overflow-hidden" style="display:none;">
                ...
                <div>
                    <div class="set-card">
                        ...
                        <div class="avatar-zone">
                            ...
                            <div class="avatar-meta">@<?= htmlspecialchars($user['username']) ?> · ID <?= str_pad($user_id, 4, '0', STR_PAD_LEFT) ?></div>
                        </div>
                    ...
                    <div class="set-card">
                        ...
                        <table class="rep-table">
                            ...
                            <tbody id="fileReportsBody">
                                <tr><td colspan="4" style="padding:20px;text-align:center;color:var(--ink-faint);font-style:italic;">Loading reports…</td></tr>
                            </tbody>
                        </table>
                    </div>
                        ...
                        <div class="card-body">
                            <div class="backup-panel">
                                ...
                                <div>
                                    ...
                                    <button class="btn btn-primary" id="backupBtn" onclick="triggerBackup()" style="width:100%;justify-content:center;">
                                        ↓ Generate &amp; Download Backup
                                    </button>
                                    <div style="margin-top:10px;font-size:10.5px;color:var(--ink-faint);line-height:1.6;">
                                        Creates a full SQL dump of the database and downloads it to your computer. A record is logged in backup history.
                                    </div>
                                </div>
                                <!-- Archive icon -->
                                <div class="backup-icon-block">
                                    ...
                                    <div style="font-family:var(--f-mono);font-size:9px;color:var(--ink-faint);text-align:center;line-height:1.8;" id="lastBackupMeta">—</div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div style="overflow-x:auto;">
                            <table class="bk-table">
                                ...
                                <tbody id="backupHistoryBody">
                                    <tr><td colspan="4" style="padding:20px;text-align:center;color:var(--ink-faint);font-style:italic;">Loading backup history…</td></tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
    ...
    function loadBackupHistory() {
        ...
        fetch('/secretary/profile/backup_history.php')
            .then(r => r.json())
            .then(data => {
                if (data.status !== 'ok' || !data.data.length) {
