...
$(function () {
    ...
    $('#viewBlotterModal').dialog(dlgCfg({
        width: 860,
        buttons: {
            'Save Changes': function () { saveBlotter(); },
            'Archive Case': function () { archiveCase($('#vc-id').val(), $('#vc-case-no').text()); },
            ...
            cases.forEach((c, i) => {
                $body.append(`<tr>
                    ...
                    <td style="text-align:center;">
                        <button class="act-btn restore-case-btn"
                            style="color:var(--ok-fg);border-color:color-mix(in srgb,var(--ok-fg) 30%,transparent);"
                            data-id="${c.id}" data-case="${esc(c.case_number)}">
                            Restore
                        </button>
                    </td>
                </tr>`);
